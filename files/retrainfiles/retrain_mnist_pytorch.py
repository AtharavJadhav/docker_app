...
def get_pytorch_model_io_names_shapes(model, input_shape):
    dummy_input = torch.randn(*input_shape)
    output = model(dummy_input)
    input_name = list(model.state_dict().keys())[0].split('.')[0]  # Assuming the first layer's name
    output_name = list(model.state_dict().keys())[-1].split('.')[0]  # Assuming the last layer's name
    input_shape = list(dummy_input.shape)
    output_shape = list(output.shape)
    return input_name, output_name, input_shape, output_shape

# Function to prepare Triton repository
def prepare_triton_repository_pytorch(model, model_name, input_shape, model_filepath):
    ...
    shutil.move(model_filepath, os.path.join(model_version_dir, "model.pt"))

    input_name, output_name, input_shape, output_shape = get_pytorch_model_io_names_shapes(model, input_shape)

    # Modify input_shape and output_shape to have -1 as the first dimension
    input_shape[0] = 1
    output_shape[0] = 1

    # Write the config.pbtxt file with the updated dimensions
    with open(os.path.join(model_dir, "config.pbtxt"), "w") as f:
        f.write(f"""
name: "{model_name}"
platform: "pytorch_libtorch"
max_batch_size: 4  # Enable dynamic batching
input [
  {{
    name: "{input_name}"
    data_type: TYPE_FP32
...
prepare_triton_repository_pytorch(model, model_name, input_shape, model_path)
print("Model loading and Triton repository preparation complete.")